...
class SocketClients {
    ...
    saveClient(pubId, userId, conversationId, client, deviceUUID, usersDetails) { 
        this.clientList = [
            ...this.clientList, 
            {
                pubId: pubId,
    ...
    removeClient(socketId, pubId) { // todo decide wtf is gonna happen with this. // Dont know what to the left means // decide if _id instead?
        const index = this.clientList.findIndex(x => x.socketId == socketId);
        if (index !== -1){
            this.clientList = [...this.clientList.slice(0, index), ...this.clientList.slice(index+1)];
            console.log("Removed a socket from clients");
            return this.clientList.find(x => x.pubId == pubId && x.socketId !== socketId); //otherClientsSameAccount 
        }
    }
    removeClientsOfUserFromConversation(pubId, conversationId) { //if kicked from convo use filter?
