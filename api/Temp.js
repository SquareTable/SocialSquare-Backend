...
const rateLimiters = {
    ...
    '/getimagecommentswithkey/:imageKey': rateLimit({
        windowMs: 1000 * 60, //1 minute
        max: 30,
        standardHeaders: false,
        legacyHeaders: false,
    ...
    '/searchforimagecommentreplies/:sentimagekey/:sentcommentid': rateLimit({
        windowMs: 1000 * 60, //1 minute
        max: 60,
        standardHeaders: false,
        legacyHeaders: false,
    ...
    '/searchforthreadcommentreplies/:sentthreadid/:sentcommentid': rateLimit({
        windowMs: 1000 * 60, //1 minute
        max: 60,
        standardHeaders: false,
        legacyHeaders: false,
...
router.post('/downvotepoll', rateLimiters['/downvotepoll'], (req, res) => {
    ...
    //Check Input fields
    if (userId == "" || pollId == "") {
        HTTPHandler.badInput(res, 'Either userId or pollId is an empty string. This is not allowed.')
    } else {
        //Find User
        User.findOne({_id: {$eq: userId}}).lean().then(result => {
            if (result) {
                ...
                Poll.findOne({_id: {$eq: pollId}}).lean().then(data => {
                    if (data) {
                        pollPostHandler.downvote(data, result).then(successMessage => {
                            ...
                            console.log('Poll Post Error Object:', error)
                            if (error.privateError) {
                                console.error('An error occured while downvoting poll post.')
                                console.error('The error was:', error.privateError)
                            }
                            HTTPHandler.serverError(res, error.publicError)
                        })
                    } else {
                        HTTPHandler.notFound(res, 'Could not find poll')
                    }
                }).catch(error => {
                    console.error('An error occured while finding user with id:', pollId)
                    console.error('The error was:', error)
...
router.get('/searchforpollcommentreplies/:sentpollid/:sentcommentid', rateLimiters['/searchforpollcommentreplies/:sentpollid/:sentcommentid'], (req, res) => {
    ...
    if (sentPollId.length == 0) {
        return HTTPHandler.badInput(res, 'sentPollId must not be an empty string.')
    }

    if (sentUserId.length == 0) {
        return HTTPHandler.badInput(res, 'sentUserId must not be an empty string.')
    }

    if (sentCommentId.length == 0) {
        return HTTPHandler.badInput(res, 'sentCommentId must not be an empty string.')
    }
                            ...
                            commentReplies.forEach(function (item, index) {
                                User.find({_id: commentReplies[index].commenterId}).then(result => {
                                    if (result.length) {
                                        ...
                                        HTTPHandler.notFound(res, 'Comment creator could not be found')
                                    }
                                    itemsProcessed++;
...
router.post('/imagepostcommentreply', rateLimiters['/imagepostcommentreply'], (req, res) => {
    const userId = req.tokenData;
    let {comment, userName, imageKey, commentId} = req.body;
    ...
    if (comment.length == 0) {
        return HTTPHandler.badInput(res, 'comment cannot be an empty string.')
    }

    if (userName.length == 0) {
        return HTTPHandler.badInput(res, 'userName cannot be an empty string.')
    }
    ...
    User.find({_id: userId}).then(result => {
        if (result.length) {
            if (result[0].name == userName) {
                ImagePost.find({imageKey: {$eq: imageKey}}).then(data => {
                    if (data.length) {
                        var comments = data[0].comments
                        async function findThreads(sentIndex) {
                            ...
                            ImagePost.findOneAndUpdate({_id: {$eq: imageKey}}, { $push: { [`comments.${sentIndex}.commentReplies`]: commentForPost } }).then(function(){
                                console.log("SUCCESS1")
                                HTTPHandler.OK(res, 'Comment upload successful')
                            })
                            .catch(err => {
                                console.error('An error occurred while adding comment:', commentForPost, 'to:', `"comments.${sentIndex}.commentReplies`, 'of image post with imageKey:', imageKey, '. The error was:', err)
                                HTTPHandler.serverError(res, 'An error occurred while adding comment. Please try again later.')
                            });
                        }
                        var itemsProcessed = 0
                        ...
                        HTTPHandler.notFound(res, 'The image post could not be found')
                    }
                })
            } else {
                HTTPHandler.badInput(res, 'userName provided does not match username in the database.')
            }
        } else {
            HTTPHandler.notFound(res, 'Could not find user with your id.')
        } 
...
router.get('/getimagecommentswithkey/:imageKey', rateLimiters['/getimagecommentswithkey/:imageKey'], (req, res) => {
    ...
    async function findComments() {
        await ImagePost.find({imageKey: {$eq: imageKey}}).then(data => {
            if (data.length) {
                var nameSendBackObject = [];
                var comments = data[0].comments;
                var itemsProcessed = 0;
                if (comments.length == 0) {
                    HTTPHandler.notFound(res, 'No comments could not be found for image post')
                } else {
                    comments.forEach(function (item, index) {
                        User.find({_id: comments[index].commenterId}).then(result => {
                            if (result.length) {
                                ...
                                nameSendBackObject.push({commentId: data[0].comments[index].commentId, commenterName: result[0].name, commenterDisplayName: result[0].displayName, commentText: data[0].comments[index].commentsText, commentUpVotes: commentUpVotes, commentReplies: data[0].comments[index].commentReplies.length, datePosted: data[0].comments[index].datePosted, profileImageKey: result[0].profileImageKey, commentUpVoted: commentUpVoted, commentDownVoted: commentDownVoted})
                            } else {
                                HTTPHandler.notFound(res, 'An error occurred while checking for comment creator')
                            }
                            itemsProcessed++;
                            if(itemsProcessed === comments.length) {
                                console.log("Before Function")
        ...
        .catch(err => {
            console.error('An error occurred while finding image post with imageKey:', imageKey, '. The error was:', err)
    ...
    async function findImages() {
        await ImagePost.find({imageKey: {$eq: sentImageKey}}).then(data => {
            if (data.length) {
                ...
                var nameSendBackObject = [];
                if (comments.length == 0) {
                    HTTPHandler.notFound(res, 'No comments found')
                } else {
                    function forAwaits(index) {
                        User.find({_id: comments[index].commenterId}).then(result => {
                            if (result.length) {
                                ...
                                HTTPHandler.notFound(res, "Couldn't find comment creator.")
                            }
                        })
                    }
                    var itemsProcessed  = 0
        ...
        .catch(err => {
            console.error('An error occurred while finding image post with image key:', sentImageKey, '. The error was:', err)
            HTTPHandler.serverError(res, 'An error occurred while finding image post. Please try again later.')
        });
    }
    findImages()
...
//search for thread comments
router.get('/searchforimagecommentreplies/:sentimagekey/:sentcommentid', rateLimiters['/searchforimagecommentreplies/:sentimagekey/:sentcommentid'], (req, res) => {
    let sentImageKey = req.params.sentimagekey
    const sentUserId = req.tokenData;
    let sentCommentId = req.params.sentcommentid
...
router.post('/postcategorywithimage', rateLimiters['/postcategorywithimage'], upload.single('image'), async (req, res) => {
    ...
    if (typeof categoryNSFW !== 'boolean' && categoryNSFW !== 'false' && categoryNSFW !== 'true') {
        deleteFile()
        return HTTPHandler.badInput(res, `categoryNSFW must be a boolean, or "false", or "true"`)
    }

    if (typeof categoryNSFL !== 'boolean' && categoryNSFL !== 'false' && categoryNSFW !== 'true') {
        deleteFile()
        return HTTPHandler.badInput(res, `categoryNSFL must be a boolean, or "false", or "true"`)
    }

    if (typeof sentAllowScreenShots !== 'boolean' && sentAllowScreenShots !== 'false' && sentAllowScreenShots !== 'true') {
        deleteFile()
        return HTTPHandler.badInput(res, `sentAllowScreenshots must be a boolean, or "false", or "true"`)
    }
    ...
    User.find({_id: {$eq: creatorId}}).then(result => {
            ...
            Category.find({categoryTitle: {'$regex': `^${categoryTitle}$`, $options: 'i'}}).then(categoryFound => {
                if (!categoryFound.length) { // category title not already used so allow it
                    imageHandler.compressImage(req.file.filename).then(imageKey => {
                        ...
                        .catch(err => {
                            imageHandler.deleteImageByKey(imageKey)
                            console.error('An error occured while saving new category:', err)
                            HTTPHandler.serverError(res, 'An error occurred while saving category!')
                        })
                    }).catch(error => {
                        console.error('An error was thrown from ImageLibrary.compressImage while compressing image with filename:', req.file.filename)
                        console.error('The error was:', error)
                ...
                deleteFile()
                console.error('An error occured while seeing if a category title already exists or not. The title to be checked was:', categoryTitle)
                console.error('The error was:', error)
                HTTPHandler.serverError(res, "An error occurred while checking if a category already has your desired category's name. Please try again later.")
            })
        } else {
            deleteFile()
        ...
        deleteFile()
        console.error('An error occured while finding user with ID: ', creatorId)
        console.error('The error was:', error)
        HTTPHandler.serverError(res, 'An error occurred while finding user. Please try again later.')
    })
})
...
router.post('/posttextthread', rateLimiters['/posttextthread'], (req, res) => {
    ...
    if (typeof threadBody !== 'string') {
        return HTTPHandler.badInput(res, `threadBody must be a string. Provided type: ${typeof threadBody}`)
    }

    if (typeof threadNSFW !== 'boolean' && threadNSFW !== "false" && threadNSFW !== "true") {
        return HTTPHandler.badInput(res, 'threadNSFW must either be a boolean, "false", or "true"')
    }

    if (typeof threadNSFL !== 'boolean' && threadNSFL !== "false" && threadNSFL !== "true") {
        return HTTPHandler.badInput(res, 'threadNSFL must either be a boolean, "false", or "true"')
    }

    if (typeof sentAllowScreenShots !== 'boolean' && sentAllowScreenShots !== "false" && sentAllowScreenShots !== "true") {
        return HTTPHandler.badInput(res, 'sentAllowScreenShots must either be a boolean, "false", or "true"')
    }
    ...
    if (threadBody.length > 1000 || threadBody.length == 0) {
        return HTTPHandler.badInput(res, 'threadBody must be between 1 and 1000 characters long')
    }
        ...
        if (result.length) {
            Category.find({categoryTitle: {$eq: threadCategory}}).then(data => {
                if (data.length) {
                    ...
                    //allowScreenShots set up
                    const allowScreenShots = data[0].allowScreenShots ? sentAllowScreenShots : false;
                    console.log(`allowScreenShots ${allowScreenShots}`)
                    ...
                    .catch(err => {
                        console.error('An error occurred while saving new thread. The error was:', err)
                        HTTPHandler.serverError(res, 'An error occurred while saving thread. Please try again later.')
                    })
                } else {
                    HTTPHandler.notFound(res, 'No category found!')
                }
            }).catch(error => {
            ...
            HTTPHandler.notFound(res, 'Could not find user with provided creatorId.')
        }
    }).catch(error => {
        console.error('An error occurred while finding user with id:', creatorId, '. The error was:', error)
        HTTPHandler.serverError(res, 'An error occurred while finding user. Please try again later.')
...
//search for thread comments
router.get('/searchforthreadcommentreplies/:sentthreadid/:sentcommentid', rateLimiters['/searchforthreadcommentreplies/:sentthreadid/:sentcommentid'], (req, res) => {
    let sentThreadId = req.params.sentthreadid
    const sentUserId = req.tokenData;
    let sentCommentId = req.params.sentcommentid
