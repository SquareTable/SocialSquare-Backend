...
router.get('/userAPI/:name', (req, res) => {
    try {
        ...
        if (name == "") {
            ...
            User.find({name: {$eq: name}}).then(userFound => {
                if (userFound.length) {
                    res.json({
                        status: "SUCCESS",
                        message: "Found.",
...
router.get('/postAPI/:name/:postname/:postformat', (req, res) => {
    try {
        ...
        if (name == "" || postName == "") {
                ...
                if (format == "Image") {
                    res.json({
                        ...
                        data: {
                            ...
                            upVotes: sortedResult[0].upVotes.length,
                            downVotes: sortedResult[0].downVotes.length,
            ...
            User.find({name: {$eq: name}}).then(userFoundResult => {
                if (userFoundResult.length) {
                    if (userFoundResult[0].privateAccount) {
                        ...
                        if (postformat == "Image") {
                            if (postName !== "None") {
                                ImagePost.find({creatorId: userFoundResult[0]._id}).then(imagePostsFound => {
                                                ...
                                                if (itemsProcessed == imagePostsFound.length) {
                                                    res.json({
                                                        ...
                                                        data: {
                                                            ...
                                                            upVotes: mostSimilarImagePost.upVotes.length,
                                                            downVotes: mostSimilarImagePost.downVotes.length,
                                        ...
                                        if (imagePostsFound.length == 1) {
                                            res.json({
                                                ...
                                                data: {
                                                    ...
                                                    upVotes: imagePostsFound[0].upVotes.length,
                                                    downVotes: imagePostsFound[0].downVotes.length,
